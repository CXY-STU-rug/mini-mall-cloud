...
> 本目录 8 份 docx 覆盖：单机并发 → 线程池 → 分布式锁 → 锁的实战应用 → Lua 原子脚本 → 锁所依赖的 Redis 本身 → 分布式事务。
...
| ⑧ | Redis Lua脚本专题_原子性利器.docx | ②③秒杀的横切点：Lua 凭什么原子(单线程) vs 锁(互斥)、EVAL/EVALSHA/KEYS·ARGV、对照项目 seckill_stock.lua 秒杀扣减 + RedisLockUtil 解锁防误删逐行 |
...
- Redis 八股 → ② 首章(5 大类型) + ④ + ⑤
- 项目深挖("你项目怎么解决超卖/缓存击穿") → ① 第 5 章 + ③ 全部 + ② 源码部分 + ⑧（Lua 秒杀扣减）
...
| Lua 凭什么原子 / 和锁有什么区别 | ⑧ 二（单线程串行 ≠ 互斥） |
| EVAL / EVALSHA / SHA1 缓存是什么 | ⑧ 三 |
| KEYS 和 ARGV 区别 / key 为什么不能硬编码 | ⑧ 3.3（集群分槽） |
| 秒杀 seckill_stock.lua 逐行 / 4 个返回码 | ⑧ 四 ↔ 代码见 order/resources/lua/seckill_stock.lua |
| Java 侧怎么调 Lua（execute 三个参数） | ⑧ 4.1 ↔ SeckillActivityServiceImpl |
| 手写分布式锁解锁为什么必须 Lua（防误删） | ⑧ 五 ↔ RedisLockUtil.unlock |
| DefaultRedisScript / 返回类型为什么必须 Long | ⑧ 六、6.1 ↔ LuaScriptConfig |
| Lua 脚本能不能死循环/随机 / CROSSSLOT / NOSCRIPT | ⑧ 七（避坑连环问） |
...
*最后更新：2026-07-29（新增⑧Redis Lua脚本专题_原子性利器，把散在②里的 Lua 独立成篇，对照 seckill_stock.lua / RedisLockUtil 逐行）*
*2026-07-19（新增⑦线程池专题_ThreadPoolExecutor深入解析，作为①的续篇）*
